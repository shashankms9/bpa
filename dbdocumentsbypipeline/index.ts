...
if (process.env.USE_LOCAL_STORAGE === 'true') {
    db = new MongoDB(process.env.AzureWebJobsStorage, process.env.MONGO_DB_DB, process.env.MONGO_DB_CONTAINER)
} else {
    //db = new CosmosDB(process.env.COSMOS_DB_CONNECTION_STRING, process.env.COSMOS_DB_DB, process.env.COSMOS_DB_CONTAINER)
    db = new BlobDB(process.env.AzureWebJobsStorage, "", 'results')
}
