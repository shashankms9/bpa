...
#### Database connections for `backend`

- `AZURE_COSMOS_MONGODB_CONNECTION_STRING` environment variable is configured in [backend.bicep](./infra/app/backend.bicep) to connect to the MongoDB database. Modify this variable to match your application's needs.
- Environment variables for connecting to Redis are available implicitly as: `REDIS_ENDPOINT`, `REDIS_HOST`, `REDIS_PASSWORD`, and `REDIS_PORT`.
...
- [app/db-cosmos.bicep](./infra/app/db-cosmos.bicep) - Azure Cosmos DB (MongoDB) to host the 'testedeploy' database.
